...
class mainHomeFragmentRepository {
        ...
        val postListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (childDataSnapshot in dataSnapshot.children) {
                    ...
                    if(favor.user == firebaseAuth.uid.toString()) {
                        favorList.add(favor)
                    }
                }
        ...
        val postListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (childDataSnapshot in dataSnapshot.children) {
                    ...
                    Log.v("MyOutA", "" + childDataSnapshot.getKey()); //displays the key for the node
                    Log.v("MyOutA", "" + favor.user);
                    if(favor.title == title && favor.status == "Entregado") {
                        database.child("favores").child(childDataSnapshot.getKey().toString()).setValue(Favor(favor.title,favor.description,favor.karma,favor.user,favor.doer,"Completado"))
                    }
                }
            }
